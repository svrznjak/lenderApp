...
function toggleHideArchived() {
  appStateStore.budgetsViewState.hideArcived = !appStateStore.budgetsViewState.hideArcived;
}

const filteredBudgets = computed(() => {
  let budgets: IBudget[] = Object.entries(budgetStore.budgets)
    .map((entry) => entry[1]);

  if (appStateStore.budgetsViewState.hideArcived) budgets = budgets.filter((budget) => budget.isArchived === false);
  if (appStateStore.budgetsViewState.searchFieldText === '') return budgets.sort(function (a, b) {
    return a.name.toLowerCase().localeCompare(b.name.toLowerCase());
  });

  return budgets.filter(budget => {
    return budget.name.toLowerCase().includes(appStateStore.budgetsViewState.searchFieldText.toLowerCase())
  }).sort(function (a, b) {
    return a.name.toLowerCase().localeCompare(b.name.toLowerCase());
  });
});
  ...
  <main id="budgets-view">
    ...
    <ScrollArea :saveScrollTop="true">
      <ContentContainer>
        ...
        <AppButton @click="toggleHideArchived"
          :styleType="appStateStore.budgetsViewState.hideArcived ? 'empty' : 'regular'">{{
        ...
        <AppCard :hasArrow="true" @click="()=>router.push({name: 'budget', params:{id: budget._id} })"
          ...
          <div style="margin-top:20px; display:flex;">
            <AppInfoBadge v-if="budget.isArchived" color="yellow">{{t('archived')}}</AppInfoBadge>
          </div>
        </AppCard>
      </ContentContainer>
    </ScrollArea>
